...
export default async function (req: RastreioRequest, res: NextApiResponse) {
  ...
  try {
    const getTrackingDataResponse = await rastreioApi.post<TrackingData>(
      "/trackingdest",
      {
        cnpj,
        nro_nf,
    ...
    const trackingDataFiltered = trackingData.tracking.reduce((acc, curr) => {
      if (curr.ocorrencia.includes("(80)")) {
        ...
        curr.ocorrencia.includes("(85)") ||
        curr.ocorrencia.includes("(84)") ||
        curr.ocorrencia.includes("(83)") ||
        curr.ocorrencia.includes("(82)")
      ) {
        acc[1] = {
          ocorrencia: curr.ocorrencia.includes("(85)")
    ...
    return res.status(200).json(newTrackingData);
  } catch (error) {
