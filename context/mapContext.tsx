...
const MapProvider = ({ children }: MapProviderProps) => {
  ...
  const combinedDistributors = useMemo(() => {
    if (filteredStores.length > 0) {
      ...
      if (selectedProductLine) {
        return sortedStores.filter(
          (store) => store.product_line === selectedProductLine.toLowerCase(),
        )
      }
      ...
      if (selectedState) {
        return DISTRIBUTORS_ITEMS.filter(
          ...
          selectedProductLine.length > 0
            ? store.product_line === selectedProductLine.toLowerCase()
            : true,
        )
      }
  ...
  }, [filteredStores, selectedCountry, selectedState, selectedProductLine])
